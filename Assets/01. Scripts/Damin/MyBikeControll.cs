...
public class MyBikeControll : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        wheels[0] = SetWheelComponent(bikeWheels.wheels.wheelFront, bikeWheels.wheels.AxleFront, false, bikeSetting.maxSteerAngle, -90);
        wheels[1] = SetWheelComponent(bikeWheels.wheels.wheelBack, bikeWheels.wheels.AxleBack, true, 0, -90);
    ...
    void Update()
    {
        ...
        if (activeControl)
        {
            ...
            if (!bikeSetting.automaticGear)
            {
                if (Input.GetKeyDown("page up") || Input.GetKeyDown(KeyCode.Alpha9))
                {
                    ShiftUp();


                }
                if (Input.GetKeyDown("page down") || Input.GetKeyDown(KeyCode.Alpha3))
                {
                    ShiftDown();
        ...
        steer2 = Mathf.LerpAngle(steer2, steer * -bikeSetting.maxSteerAngle, Time.deltaTime * 9.0f);
    ...
    void FixedUpdate()
    {
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if (brake || accel < 0.0f)
            {
                if ((accel < 0.0f) || (brake && w == wheels[1]))
                {
                    ...
                    wantedRPM = 0.0f;
                    col.brakeTorque = bikeSetting.brakePower * 100;
                    w.rotation = w_rotate;
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            float SteerAngle = Mathf.Clamp((speed / 10000) / bikeSetting.maxSteerAngle, 1.0f, bikeSetting.maxSteerAngle);
    ...
    private void BikeCurrentSpeed()
    {
        CurrentSpeed = myRigidbody.velocity.magnitude * 3.6f;
    }
